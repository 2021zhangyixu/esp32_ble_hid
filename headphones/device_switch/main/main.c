    ...
    if (sec_conn) {
        switch (*value) {
            case '0':{
                ...
                // 0. Pause and Play key
                // 1. Increase brightness    2. Decrease brightness
                // 3. Next track             4. Previous track
                // 5. Increase volume        6. Decrease volume
                // 7. Reserved
                hid_headphones_control(hid_conn_id[remote_conn_id],(1 << 0));
                break;
            }
        }
    }
...
{
    switch (event) {
        case ESP_HIDD_EVENT_REG_FINISH: {  // HID GATT service registration complete event
        ...
        case ESP_BAT_EVENT_REG: {  // Battery level GATT service registration complete event
            ESP_LOGI(HID_DEMO_TAG, "ESP_BAT_EVENT_REG");
            break;
        }
        case ESP_HIDD_EVENT_BLE_CONNECT: { // HID GATT service connection event
        ...
        case ESP_HIDD_EVENT_BLE_DISCONNECT: { // HID GATT service disconnection event
            sec_conn = false;
